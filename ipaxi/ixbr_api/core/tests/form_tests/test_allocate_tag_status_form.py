...
class AllocateTagStatusFormTestCase(TestCase):
    def setUp(self):
        ...
        status = ['ALLOCATED', 'AVAILABLE', 'AVAILABLE', 'PRODUCTION']
        self.tag = mommy.make(
            Tag,
    ...
    def test_allocate_tag_status_form_when_status_code_200_and_context(self):
        request = self.client.get(
            reverse('core:allocate_tag_status', args=[self.ix.code])
        )
        self.assertEqual(request.status_code, 200)
        self.assertEqual(request.context['ix'], self.ix.code)
        self.assertTrue(request.context['form'].fields['tag_to_allocate'])

    def test_post_allocate_tag_without_bundle(self):
        self.assertEqual(self.tag[1].status, 'AVAILABLE')
        response = self.client.post(
            reverse('core:allocate_tag_status', args=[self.ix.code]),
            {'tag_to_allocate': self.tag[1].pk}
        )
        tag_allocated = Tag.objects.get(uuid=self.tag[1].uuid)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(tag_allocated.status, 'ALLOCATED')
    ...
    def test_post_allocate_tag_with_bundle(self):
        ...
        response = self.client.post(
            ...
            {'tag_to_allocate': tag_set[0].pk}
        )

        self.assertEqual(response.status_code, 302)

        tag_modified = Tag.objects.get(uuid=tag_set[0].uuid)
        self.assertEqual(tag_modified.status, 'ALLOCATED')
