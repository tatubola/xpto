...
class ReleasePortResourceFormTestCase(TestCase):
    def setUp(self):
        ...
        self.ports[0].reserve_this()
        self.ports[1].reserve_this()

    def test_template(self):
        request = self.client.get(reverse(
            "core:release_port_resource", args=[self.switch.uuid]))
        self.assertEqual(request.status_code, 200)
        self.assertTemplateUsed('forms/release_port_resource.html')

    def test_release_port_resource_form_success(self):
        self.response = self.client.post(
            reverse('core:release_port_resource', args=[self.switch.pk]),
            {'ports_to_release': [self.ports[0].uuid, self.ports[1].uuid]})

        self.assertEqual(self.response.status_code, 302)
        messages = [
            m.message for m in get_messages(self.response.wsgi_request)]
        self.assertIn('Port(s) Released', messages)
