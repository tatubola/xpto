...
class ReleaseTagStatusFormTestCase(TestCase):
    def setUp(self):
    ...
    def test_release_tag_recourse_form_when_status_code_200_and_context(self):
        request = self.client.get(
            reverse('core:release_tag_resource', args=[self.ix.code])
        )
        self.assertEqual(request.status_code, 200)
        self.assertEqual(request.context['ix'], self.ix.code)
        self.assertTrue(request.context['form'].fields['tag_to_release'])

    def test_reserve_tag_recourse_form_without_bundle(self):
        self.assertEqual(self.tag[0].reserved, True)
        response = self.client.post(
            reverse('core:release_tag_resource', args=[self.ix.code]),
            {'tag_to_release': self.tag[0].pk}
        )
        tag_release = Tag.objects.get(uuid=self.tag[1].uuid)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(tag_release.reserved, False)
