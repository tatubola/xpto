...
from ...forms import GenericMLPAUsedChannelForm
...
class GenericMLPAUsedFormTestCase(TestCase):
    def setUp(self):
    ...
    def test_when_status_code_200_and_context(self):
        request = self.client.get(
            reverse('core:generic_mlpa_add_form',
                    args=[self.asn.number,
                          self.customer_channel.uuid,
                          self.ixs[0].code]))

        self.assertEqual(200, request.status_code)
        self.assertTemplateUsed('forms/generic_mlpa_used_channel.html')

        self.assertEqual(
            request.context['channel'], str(self.customer_channel.uuid))
        self.assertEqual(request.context['code'], self.ixs[0].code)
        self.assertEqual(request.context['asn'], str(self.asn.number))
        self.assertEqual(
            request.context['customer_channel'], self.customer_channel)
        ...
        request = self.client.post(
            reverse('core:generic_mlpa_add_form',
                    args=[self.asn.number,
                          ...
                          self.ixs[0].code]),
            {'last_ticket': 1234,
             'channel': self.customer_channel.uuid,
    ...
    def test_add_ipv4_from_another_ix(self):
        ...
        self.assertEqual(
            message,
            GenericMLPAUsedFormTestCase.message_ip_another_ix(
    ...
    def test_ipv4_and_ipv6_from_another_ix(self):
        request = self.client.post(
            reverse('core:generic_mlpa_add_form',
                    args=[self.asn.number,
                          self.customer_channel.uuid,
        ...
        message = request.context['message']
        self.assertEqual(
            message, GenericMLPAUsedFormTestCase.message_ips_another_ix(
                self.ixs[0].code))
    ...
    def test_ipv4_not_found(self):
        ...
        message = request.context['message']
        self.assertEqual(
            message, GenericMLPAUsedFormTestCase.message_ip_not_found(
                'IPv4'))
    ...
    def test_mlpa_used_channel_form_fail(self):
        form = GenericMLPAUsedChannelForm(
            data={
                'last_ticket': 1234,
                'channel': self.customer_channel.uuid,
                'service_option': 'v4_and_v6',
